...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    x, y = opponent_score // 10 % 10, player_score % 10
    diff = 3 * abs(x - y)
    if diff > 1:
        return diff
    return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        score1 = 0
        while score1 < goal:
            if strategy(score0, score1) != basis:
                return False
            score1 += 1
        score0 += 1
    return True
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    total = sus_update(0, score, opponent_score, dice=six_sided)
...
def final_strategy(score, opponent_score):
    """This strategy:
    1. Check whether you can win by rolling 0, 1 or 2 dice(s). 
    2. If you are in the lead, roll 0 whenever it would give you more points on average than rolling 6.
    """
    # BEGIN PROBLEM 12
    win_threshold = GOAL - score

    i = 0
    while i < 3:
        if sus_update(i, score, opponent_score) >= win_threshold:
            return i
        i += 1

    roll0 = sus_update(0, score, opponent_score)
    roll6 = sus_update(6, score, opponent_score)    
    if score > opponent_score:
        if roll0 >= roll6:
            return 0
