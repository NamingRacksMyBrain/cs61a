...
def interleaved_sum(n, f_odd, f_even):
    ...
    def f(k):
        ...
        if k == n:
            ...
            return 0
        return f_odd(k) + f_even(k + 1) + f(k + 2)
    return f(1)
...
def count_dollars(total):
    """Return the number of ways to make change for total using 1, 5, 10, 20, 50, and 100 dollar bills.
    ...
    True
    """
    def f(total, largest_bill):
        """Return the number of ways to make change for total using MAX as max dollar bills."""
        if total < 0 or largest_bill == None:
            return 0
        if total == 0:
            return 1
        without_largest = f(total, next_smaller_dollar(largest_bill))
        with_largest = f(total - largest_bill, largest_bill)
...
def count_dollars_upward(total):
    ...
    True
    """
    def f(total, smallest_bill):
        """Return the number of ways to make change for total using MAX as max dollar bills."""
        if total < 0 or smallest_bill == None:
            return 0
        if total == 0:
            return 1
        with_smallest = f(total - smallest_bill, smallest_bill)
        without_smallest = f(total, next_larger_dollar(smallest_bill))
